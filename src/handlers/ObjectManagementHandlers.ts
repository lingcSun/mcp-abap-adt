...
export class ObjectManagementHandlers extends BaseHandler {
    ...
    switch (toolName) {
      ...
      default:
        throw new McpError(
          ...
          `Unknown object management tool: ${toolName}`,
        );
    }
  }
  ...
  async handleActivateObjects(args: any): Promise<any> {
    ...
    try {
      if (
        !args.objects ||
        (typeof args.objects !== "string" && !Array.isArray(args.objects))
      ) {
        throw new McpError(
          ErrorCode.InvalidParams,
          "objects parameter must be a JSON string",
        );
      }
      ...
      try {
        ...
        objects.forEach((obj, index) => {
          if (
            ...
            !obj["adtcore:type"] ||
            !obj["adtcore:name"] ||
            !obj["adtcore:parentUri"]
          ) {
            throw new Error(
              `Object at index ${index} is missing required properties`,
            );
          }
        });
  ...
  async handleActivateByName(args: any): Promise<any> {
    ...
    try {
      if (!args.objectName || !args.objectUrl) {
        throw new McpError(
          ErrorCode.InvalidParams,
          "objectName and objectUrl parameters are required",
        );
      }
  ...
  async handleInactiveObjects(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const result: InactiveObjectRecord[] =
        await this.adtclient.inactiveObjects();
      this.trackRequest(startTime, true);
      return {
        content: [
          {
