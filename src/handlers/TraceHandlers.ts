...
export class TraceHandlers extends BaseHandler {
  ...
  async handleTracesHitList(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const hitList = await this.adtclient.tracesHitList(
        args.id,
        args.withSystemEvents,
      );
      this.trackRequest(startTime, true);
      return {
        content: [
  ...
  async handleTracesDbAccess(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const dbAccess = await this.adtclient.tracesDbAccess(
        args.id,
        args.withSystemEvents,
      );
      this.trackRequest(startTime, true);
  ...
  async handleTracesStatements(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const statements = await this.adtclient.tracesStatements(
        args.id,
        args.options,
      );
      this.trackRequest(startTime, true);
  ...
  async handleTracesCreateConfiguration(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const result = await this.adtclient.tracesCreateConfiguration(
        args.config,
      );
      this.trackRequest(startTime, true);
      return {
