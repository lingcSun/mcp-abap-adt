...
export class AtcHandlers extends BaseHandler {
  ...
  async handleCreateAtcRun(args: {
    ...
    const startTime = performance.now();
    try {
      const result = await this.adtclient.createAtcRun(
        args.variant,
        args.mainUrl,
        args.maxResults ?? 100,
      );
  ...
  async handleAtcWorklists(args: {
    ...
    const startTime = performance.now();
    try {
      const result = await this.adtclient.atcWorklists(
        args.runResultId,
        args.timestamp || 0,
        args.usedObjectSet || "",
  ...
  async handleAtcRequestExemption(args: {
  ...
  async handleAtcChangeContact(args: {
    itemUri: string;
    userId: string;
  }): Promise<any> {
    const startTime = performance.now();
    try {
      const result = await this.adtclient.atcChangeContact(
        args.itemUri,
        args.userId,
      );
      this.trackRequest(startTime, true);
