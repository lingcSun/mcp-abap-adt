...
export class RefactorHandlers extends BaseHandler {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            range: {
              type: "string",
              description:
                'The range to extract, as a JSON string, e.g. {"start":{"line":1,"column":0},"end":{"line":5,"column":10}}',
            },
          },
          required: ["uri", "range"],
        },
        ...
        inputSchema: {
          ...
          properties: {
            proposal: {
              type: "string",
              description:
                "The extract method proposal returned by extractMethodEvaluate, as a JSON string.",
            },
          },
          required: ["proposal"],
        },
        ...
        inputSchema: {
          ...
          properties: {
            refactoring: {
              type: "string",
              description:
                "The refactoring returned by extractMethodPreview, as a JSON string.",
            },
          },
    ...
    switch (toolName) {
      ...
      default:
        throw new McpError(
          ...
          `Unknown refactor tool: ${toolName}`,
        );
    }
  }
  ...
  async handleExtractMethodEvaluate(args: any): Promise<any> {
    ...
    try {
      ...
      const result = await this.adtclient.extractMethodEvaluate(
        args.uri,
        range,
      );
  ...
  async handleExtractMethodPreview(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const proposal = this.parseObjectArg<ExtractMethodProposal>(
        args.proposal,
        "proposal",
      );
  ...
  async handleExtractMethodExecute(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      const refactoring = this.parseObjectArg<GenericRefactoring>(
        args.refactoring,
        "refactoring",
      );
